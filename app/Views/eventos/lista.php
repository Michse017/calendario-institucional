...
$qs = array_filter(['anio' => $f('anio'), 'area_id' => $f('area_id'), 'tipo_accion_id' => $f('tipo_accion_id'), 'segmento_id' => $f('segmento_id'), 'estado' => $f('estado'), 'persona' => $f('persona'), 'q' => $f('q'), 'mios' => $mios ? '1' : '', 'orden' => $orden === 'desc' ? 'desc' : '']);
$u = usuario_actual();
?>
...
<form method="get" class="card mb-5 grid gap-3 p-4 md:grid-cols-10" x-data="buscadorEventos()">
  ...
  <select class="input" name="persona" aria-label="<?= h(t('Responsable')) ?>"><option value=""><?= h(t('Cualquier responsable')) ?></option><?php foreach ($usuarios as $us): ?><option value="<?= (int) $us['id'] ?>" <?= $f('persona') == $us['id'] ? 'selected' : '' ?>><?= h($us['nombre']) ?></option><?php endforeach; ?></select>
